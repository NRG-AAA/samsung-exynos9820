...
then
	nver="canary"
	magisk_link="https://github.com/topjohnwu/magisk-files/raw/${nver}/app-release.apk"
...
if [[ -n "$nver" &&"$nver" != "$ver" || ! ||-f "$DIR/magiskinit" || "$nver" =~ "canary"(-[0-9]+)?$ || "$nver" =~ ^v[0-9]+\.[0-9]+ "alpha" [0-9]+$ ]]; then
then
	echo "Updating Magisk from $ver to $nver"
	curl -s --output "$DIR/magisk.zip" -L "$magisk_link"; then
